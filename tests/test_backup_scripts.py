...
class BackupScriptTests(unittest.TestCase):
    def test_backup_and_restore_round_trip(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            created = subprocess.run([str(ROOT / "backup.sh")], env=env, check=True, capture_output=True, text=True)
            ...
            restored = subprocess.run([str(ROOT / "restore.sh"), str(backups[0])], env=env, check=True, capture_output=True, text=True)
